...
const { mnemonicToAccount, toAccount, privateKeyToAccount } = require("viem/accounts");
const {
  createWalletClient,
  decodeEventLog,
  fromHex,
...
const HUB_URL = 'hub.pinata.cloud'; // URL + Port of the Hub
...
const SIGNER_PRIVATE_KEY = zeroAddress; // Optional, using the default means a new signer will be created each time
...
const account = privateKeyToAccount(ACCOUNT_KEY_PRIVATE_KEY);
...
const getOrRegisterSigner = async (fid) => {
...
const registerFname = async (fid) => {
  ...
  const userNameProofSignatureClaim = await signer.signUserNameProofClaim({
    name: fname,
    timestamp: BigInt(timestamp),
    owner: account.address,
  });
  const buffer = Buffer.from(userNameProofSignatureClaim.value);
  // Convert Buffer to a hex string
  const userNameProofSignature = '0x' + (buffer.toString('hex'));
...
async function setupHubbleValues() {
